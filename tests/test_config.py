...
class ConfigTests(unittest.TestCase):
    def test_config_defaults_are_paper_safe(self) -> None:
        ...
        self.assertGreaterEqual(config.max_total_risk_pct, 0.015)
        self.assertLessEqual(config.max_total_risk_pct, 0.04)
        self.assertGreaterEqual(config.energy_bucket_risk_pct, 0.006)
        self.assertLessEqual(config.energy_bucket_risk_pct, 0.016)
        self.assertGreaterEqual(config.grains_bucket_risk_pct, 0.005)
        self.assertLessEqual(config.grains_bucket_risk_pct, 0.014)
        self.assertGreaterEqual(config.softs_bucket_risk_pct, 0.004)
        self.assertLessEqual(config.softs_bucket_risk_pct, 0.011)
        self.assertGreaterEqual(config.profit_lock_trigger_pct, 1.5)
        self.assertLessEqual(config.profit_lock_trigger_pct, 5.0)
        self.assertGreaterEqual(config.profit_lock_pullback_pct, 0.75)
